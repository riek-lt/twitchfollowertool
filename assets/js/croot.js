...
var TWITCH_CLIENT_ID = 'rnqe5fseibqp7ub2017x2fd8jfrk5s';
var TWITCH_REDIRECT_URL = "https://riek-lt.github.io/twitchfollowertool/";
var localUserToken = '';
var localUser = {};
var usersInfo = {};
var followerList = [];
...
function TwitchAPIURL() {
	...
	// Quick return if we have already decided
	if(requestTimings.isDecided !== null) {
		return {'type': requestTimings.isDecided, 'url': APIURLs[requestTimings.isDecided]};
	}

	let type = 'remote';
	// Check if it's time to decide
	if(requestTimings.remote.requests >= 10 && requestTimings.local.requests >= 10) {
		let remoteAVG = requestTimings.remote.durations / requestTimings.remote.requests;
		let localAVG = requestTimings.local.durations / requestTimings.local.requests;
		if(remoteAVG > localAVG) {
			type = 'local';
		} else {
			type = 'remote';
		}
		requestTimings.isDecided = type;
		console.log('Decided that ' + type + ' is better: ' + remoteAVG + ' (remote) vs ' + localAVG + ' (local)');
	} else {
		if(requestTimings.remote.requests > requestTimings.local.requests) {
			type = 'local';
		} else {
			type = 'remote';
		}
		...
		success: function(result, textStatus, jqXHR) {
			...
			if(typeof result['data'] !== 'undefined' && typeof result['data'][0] !== 'undefined') {
				...
				if(typeof optOutChannelIDs[localUser.id] === 'boolean' && optOutChannelIDs[localUser.id] === true) {
					...
					// Bind abort button
					$('#abort-loading-button').bind('click', function(e) {
						e.preventDefault();
						stopLoading = true;
					});
				}
	...
	$.ajax({
		...
		success: function(result, textStatus, jqXHR) {
			...
			if(typeof result['data'] !== 'undefined') {
				...
				$('#follower-loading-status').text('page ' + (new Intl.NumberFormat().format(getFollowersRequestCount)) + ' / ' + (new Intl.NumberFormat().format(Math.ceil(result['total'] / 100))) + ' -> ' + (new Intl.NumberFormat().format(followerList.length)) + ' / ' + (new Intl.NumberFormat().format(result['total'])));
...
function fetchKnownBots() {
	$.ajax({
		url: '/blocklist_manager.php?preset=known_bot_users&object=true',
		type: 'GET',
		timeout: 30000,
		success: function(result, textStatus, jqXHR) {
...
function dateTimeString(unixtime) {
	let currentDate = new Date(unixtime);
...
function renderFollowerList(filter) {
	...
	html += '<table class="table table-bordered table-hover table-sm tablesorter"><thead><tr><th scope="col">Username</th><th scope="col">Followed at</th><th scope="col">Account created at</th><th scope="col">Broadcaster type</th><th scope="col">Account type</th><th scope="col">Channel views</th><th scope="col">Default logo</th><th scope="col">Is known bot</th><th scope="col">Links</th></tr></thead><tbody>';
	$(followerList).each(function(i, e) {
		...
		if(typeof filter['followedAtMin'] !== 'undefined' && e.followCreatedAt < filter['followedAtMin']) {
			...
			return;
		} else if(typeof filter['channelViewsMin'] !== 'undefined' && (typeof usersInfo[e.userID] === 'undefined' || typeof usersInfo[e.userID]['viewCount'] === 'undefined' || usersInfo[e.userID]['viewCount'] < filter['channelViewsMin'])) {
			return;
		} else if(typeof filter['channelViewsMax'] !== 'undefined' && (typeof usersInfo[e.userID] === 'undefined' || typeof usersInfo[e.userID]['viewCount'] === 'undefined' || usersInfo[e.userID]['viewCount'] > filter['channelViewsMax'])) {
			return;
		...
		html += '<tr class="follower-card' + knownBot + '" data-userid="' + escapeHtml(e.userID) + '">';
			...
			html += '</td>';
			html += '<td>';
			if(typeof usersInfo[e.userID] !== 'undefined' && typeof usersInfo[e.userID]['viewCount'] !== 'undefined') {
				html += escapeHtml(new Intl.NumberFormat().format(usersInfo[e.userID]['viewCount']));
			} else {
				html += '<i>Unknown</i>';
			}
			html += '</td>';
			...
			html += '<td>';
				...
				html += '<a href="/followerlist_viewer.php?channel=' + escapeHtml(e.userName) + '" target="_blank" rel="noopener">Followers</a> / ';
				html += '<a href="/followinglist_viewer.php?username=' + escapeHtml(e.userName) + '" target="_blank" rel="noopener">Following</a>';
			html +='</td>';
		html += '</tr>';
...
function renderFollowerListFilter() {
	...
	let html = '<div class="row pl-4">';
		html += '<div id="filter">';
			...
			html += '<div><b>Channel views</b> between <input type="text" class="filter-input" id="filter-channelViews-min" name="channelViews-min" value="" autocomplete="off"> and <input type="text" class="filter-input" id="filter-channelViews-max" name="channelViews-max" value="" autocomplete="off"></div>';
			...
			html += '</select></div>';
			html += '<div>Is disabled<abbr title="Deleted or suspended">*</abbr>: <select type="text" class="filter-input" id="filter-disabled" name="isDisabled" autocomplete="off">';
				html += '<option value="" selected>Any</option>';
				html += '<option value="yes">Yes</option>';
				html += '<option value="no">No</option>';
			html += '</select></div>';
			html += '<div>Is known bot: <select type="text" class="filter-input" id="filter-known-bot" name="known-bot" autocomplete="off">';
				html += '<option value="" selected>Any</option>';
				html += '<option value="yes">Yes</option>';
				html += '<option value="no">No</option>';
			html += '</select></div>';
	...
	$('#export-button').bind('click', function(e) {
		e.preventDefault();
		exportFollowerListAsCSV();
	});

	$('#export-filtered-button').bind('click', function(e) {
		e.preventDefault();
...
function checkDateTimeValid(id) {
	if($('#' + id).val().trim() == '' || moment($('#' + id).val().trim()).isValid()) {
...
function renderTopDayStats() {
	...
	$(followerList).each(function(i, e) {
		...
		let parsedFollowCreatedAtDate = new Date(e.followCreatedAt);
		let followDay = parsedFollowCreatedAtDate.getFullYear() + '-' + (((parsedFollowCreatedAtDate.getMonth()+1) < 10) ? '0' : '') + (parsedFollowCreatedAtDate.getMonth()+1) + '-' + ((parsedFollowCreatedAtDate.getDate() < 10) ? '0' : '') + parsedFollowCreatedAtDate.getDate();
		if(typeof followsPerDay[followDay] === 'undefined') followsPerDay[followDay] = 0;
		followsPerDay[followDay]++;

		// Acc creation per day counter
		if(typeof usersInfo[e.userID] !== 'undefined' && typeof usersInfo[e.userID]['createdAt'] !== 'undefined') {
			let parsedAccCreatedAtDate = new Date(usersInfo[e.userID]['createdAt']);
			let accCreateDay = parsedAccCreatedAtDate.getFullYear() + '-' + (((parsedAccCreatedAtDate.getMonth()+1) < 10) ? '0' : '') + (parsedAccCreatedAtDate.getMonth()+1) + '-' + ((parsedAccCreatedAtDate.getDate() < 10) ? '0' : '') + parsedAccCreatedAtDate.getDate();
	...
	$.each(followsPerDay, function(i, e) {
		let followInfo = {day: i, follows: e};
	...
	$.each(accCreatesPerDay, function(i, e) {
		let accCreateInfo = {day: i, accounts: e};
...
function filterFollowerList() {
	...
	if($('#filter-channelViews-min').val().trim() != '') {
		filter['channelViewsMin'] = $('#filter-channelViews-min').val().trim();
	}
	if($('#filter-channelViews-max').val().trim() != '') {
		filter['channelViewsMax'] = $('#filter-channelViews-max').val().trim();
	}
...
function exportFollowerListAsCSV() {
	let exportFilename = 'followerlist_' + localUser.login + '__' + moment().format('YYYY-MM-DD_HH-mm-ss') + '.csv';
	let csv = 'userName,userID,accCreatedAt,followCreatedAt,broadcasterType,accountType,channelViews,defaultLogo,isKnownBot' + "\r\n";
	$(followerList).each(function(i, e) {
		...
		csv += ',';
		if(typeof usersInfo[e.userID] !== 'undefined' && typeof usersInfo[e.userID]['viewCount'] !== 'undefined') {
			csv += usersInfo[e.userID]['viewCount'];
		} else {
			csv += 'unknown';
		}
		csv += ',';
...
function exportFilteredFollowerListAsCSV() {
	let exportFilename = 'followerlist_filtered_' + localUser.login + '__' + moment().format('YYYY-MM-DD_HH-mm-ss') + '.csv';
	let csv = 'userName,userID,accCreatedAt,followCreatedAt,broadcasterType,accountType,channelViews,defaultLogo,isKnownBot' + "\r\n";
	let userIds = {};
...
function showChannelSelect() {
	...
	// Bind lookup button
	$('#lookup-user-button').bind('click', function(e) {
		e.preventDefault();
...
$(function() {
	...
	let access_token = getQueryVariable(window.location.hash.slice(1), 'access_token');
